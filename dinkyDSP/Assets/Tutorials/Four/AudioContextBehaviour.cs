 ...
 * Tutorial four
 * 
 * This tutorial shows how you might use Dinky for a more practical purpose
 * 
 ...
 *
 * The code in the custom audio node should be fairly self explainatory - when you press the button several parameters are
 * randomised, and the position in the envelope is reset, the envelope shapes the amplitude of the audio signal over time
 * fading it out. 
 * 
	...
	public class AudioContextBehaviour : MonoBehaviour 
	{
		...
		class RayGun : IAudioNode
		{
			...
			public RayGun()
			{
				...
				m_nestedRootNode = m_gainer; //for clarity really.
			}
			...
			public double GetSample ()
			{
				...
				//(amplitude)...
				m_envelope.Position = m_position.GetSample();
				...
				var audio = m_nestedRootNode.GetSample();
